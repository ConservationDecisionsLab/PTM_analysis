...
for(it in 1:MC){
  ...
  # samples[,it] <- rlong.std.wide$Best.Guess
  
  temp <- cbind(rlong.std.wide[,1:3], samples[,it])
  names(temp)[4] <- "MC.Value"
  
  temp.wide <- spread(temp, key = Strategy, value = MC.Value)
  temp.strat <- select(temp.wide, as.character(strat.levels[2]):as.character(strat.levels[length(strat.levels)])) #select(temp.wide, S1:S22)
